...
XNVCTRL_LIB := libXNVCtrl/libXNVCtrl.a
...
ALL_CFLAGS = -m32 -std=c99 $(CFLAGS) $(LOCAL_CFLAGS) $(X11_CFLAGS) $(GTK_CFLAGS) $(INC_FLAGS)
ALL_LDFLAGS = -m32 $(LD_RUN_FLAG) $(GTK_LDFLAGS) -lm -ldl $(X11_LIBS) $(LDFLAGS)

CPPFLAGS = $(ALL_CFLAGS)


NVIDIA_SETTINGS = nvidia-probe
NVIDIA_SETTINGS_VERSION = 0.1
...
clean clobber:
	rm -rf $(OBJS_DIR) $(DEPS_DIR) $(STAMP_C)
	#$(NVIDIA_SETTINGS)
	find . -name "*~" -exec rm -f \{\} \;
